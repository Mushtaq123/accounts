...
public class MainActivity extends Activity {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		public boolean onContextItemSelected(MenuItem item) {
			...
			if(item.getTitle()=="Delete Entry"){
	            ...
	            alertDialog.setMessage("Are you sure you want delete this user's records?");
	            alertDialog.setPositiveButton("YES", new DialogInterface.OnClickListener() {
	                public void onClick(DialogInterface dialog,int which) {	     
	...
	public void onActivityResult(int requestCode, int resultCode,Intent data){
		...
		/*else if(requestCode==2){		//onClick listview item,amount addsub UI update + database update both here
			if(resultCode==RESULT_OK){
				String action=data.getStringExtra("action");
				String amount=data.getStringExtra("amount");
				String tempamount=amount;
				String reason=data.getStringExtra("reason");
				String username=str.get(positionclicked).getName();
				Integer int_amount=Integer.parseInt(amount);
				Integer int_present=Integer.parseInt(str.get(positionclicked).getAmount());
				int total=55555;			
				CommentsDataSource cd=new CommentsDataSource(this);
				if(action.equalsIgnoreCase("add")){
					total=int_amount+int_present;					
				}
				else if(action.equalsIgnoreCase("sub")){					
					total=int_present-int_amount;
					tempamount="-"+tempamount;
				}
				String time[]=new String[1];
				time[0]=getTime();
				cd.updateamount(total,username,time[0]);
				//database2 also								
				cd.insert_to_database_registration_two(username,tempamount,reason,time);
				//update str
				details temp=str.get(positionclicked);				
			...
			if(resultCode==RESULT_OK){
				String color=data.getStringExtra("color");int assdjf=0;
